...
import { getApiKeyStatus, saveApiKeys, KeyStatus } from '../lib/tauri-bridge';
    ...
    <div className="min-h-screen bg-white p-8 max-w-2xl mx-auto">
      ...
      {isFirstLaunch && (
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded text-sm text-blue-900">
          ...
          <p className="mt-3 text-xs">两边都有免费额度。Key 保存在 macOS 钥匙串(系统加密),不出现在任何文件里。</p>
        </div>
      )}
      ...
      <div className="space-y-5">
        <div>
          ...
          <input
            type="password"
            value={aliyun}
            onChange={(e) => setAliyun(e.target.value)}
            disabled={saving}
        ...
        <div>
          ...
          <input
            type="password"
            value={minimax}
            onChange={(e) => setMinimax(e.target.value)}
            disabled={saving}
            className="w-full px-3 py-2 border border-gray-300 rounded font-mono text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100"
